...
const ProductCard = ({
  ...
  product: Product;
  handleOpenEditDrawer: (product: Product) => void;
}) => {
  ...
  return (
    <Card className="relative">
      ...
      <Badge variant="default" className="absolute top-2 right-2 font-bold">
        {product.tsku}
      </Badge>
      <CardFooter className="flex justify-between items-center">
        <Button variant="outline" onClick={() => handleOpenEditDrawer(product)}>
          Edit
        </Button>
      ...
      {/* Delete Modal */}
      {openDeleteModal && <DeleteModal open={openDeleteModal} setOpen={setOpenDeleteModal} type="product" id={product.tsku} />}
    </Card>
  );
};
