...
const Styles = () => {
  ...
  return (
    <Layout>
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3 sm:gap-4">
                {data.items.map((style: Style) => (
                  <StyleCard key={style.style_id} style={style} handleOpenEditDrawer={handleOpenEditDrawer} />
                ))}
              </div>
            ) : (
              <NoData item="colors" />
      ...
      {/* Product Drawer */}
      {isDrawerOpen && (
        <StyleDrawer isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} data={styleToEdit as (CreateStyle & { style_id: number }) | null} />
      )}
    </Layout>
  );
};
