...
const Colors = () => {
  ...
  return (
    <Layout>
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3 sm:gap-4">
                {data.items.map((color: Color) => (
                  <ColorCard key={color.color_id} color={color} handleOpenEditDrawer={handleOpenEditDrawer} />
                ))}
              </div>
            ) : (
              <NoData item="colors" />
      ...
      {/* Product Drawer */}
      {isDrawerOpen && <ColorDrawer isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} data={colorToEdit} />}
    </Layout>
  );
};
